...
static void http_task(void *pvParameters)
{
	...
	char endpoint[128];
	ESP_LOGI(TAG, "CONFIG_HTTP_ENDPOINT=[%s]", CONFIG_HTTP_ENDPOINT);
	convert_mdns_host(CONFIG_HTTP_ENDPOINT, endpoint);
	ESP_LOGI(TAG, "endpoint=[%s]", endpoint);

	wait_enter("Creating new foder on Webdav Server. Press Enter when ready.");
	// Perfome MKCOL
	http_webdav_mkcol(endpoint, "/new_folder");
	// Perfome PROPFIND
	http_webdav_propfind(endpoint);

	wait_enter("Creating new file on Webdav Server. Press Enter when ready.");
	// Perfome PUT
	http_webdav_put(endpoint, "/new_folder/file.txt");
	// Perfome PROPFIND
	http_webdav_propfind(endpoint);

	wait_enter("Geting file on Webdav Server. Press Enter when ready.");
	// Perfome GET
	http_webdav_get(endpoint, "/new_folder/file.txt");

	wait_enter("Copying file on Webdav Server. Press Enter when ready.");
	// Perfome COPY
	http_webdav_copy(endpoint, "/new_folder/file.txt", "/new_folder/file2.txt", false);
	// Perfome PROPFIND
	http_webdav_propfind(endpoint);

	wait_enter("Moveing file on Webdav Server. Press Enter when ready.");
	// Perfome MOVE
	http_webdav_move(endpoint, "/new_folder/file.txt", "/new_folder/file3.txt", false);
	// Perfome PROPFIND
	http_webdav_propfind(endpoint);

	wait_enter("Copying folder on Webdav Server. Press Enter when ready.");
	// Perfome COPY
	http_webdav_copy(endpoint, "/new_folder", "/copy_folder", false);
	// Perfome PROPFIND
	http_webdav_propfind(endpoint);

	wait_enter("Deleting file on Webdav Server. Press Enter when ready.");
	// Perfome COPY
	http_webdav_delete(endpoint, "/new_folder/file3.txt");
	// Perfome PROPFIND
	http_webdav_propfind(endpoint);

	wait_enter("Deleting folder on Webdav Server. Press Enter when ready.");
	// Perfome COPY
	http_webdav_delete(endpoint, "/new_folder");
	http_webdav_delete(endpoint, "/copy_folder");
	// Perfome PROPFIND
	http_webdav_propfind(endpoint);

	ESP_LOGI(TAG, "Finish http task");
	vTaskDelete(NULL);
}
#endif
...
void app_main(void)
{
	...
	xTaskCreate(&http_task, "HTTP", 1024*4, NULL, 5, NULL);
